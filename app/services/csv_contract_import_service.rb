class CsvContractImportService
  ...
  def create_and_return_owners(emails)
    ...
    ContractOwner.upsert_all(
      owners_hashes,
      unique_by: %i[email index_contract_owners_on_email],
      returning: %i[email id],
      update_only: %i[first_name last_name]
    )
  end
  ...
  def upsert_contracts(owners, batch)
    ...
    result = Contract.upsert_all(
      valid_contract_hashes,
      unique_by: %i[external_contract_id index_contracts_on_external_contract_id],
      returning: %i[name start_date end_date value_cents supplier contract_owner_id]
    )
